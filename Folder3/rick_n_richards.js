...
Rick eats with speed twice that of Richard. 
    If a player starts to eat the banana, other player can't touch it. 
...
As Rick's speed is twice Richard's, He will start with second banana. 
    Richard still would be eating the first. 
    Rick will start with his third banana and when Richard will finish his, he'l start with his second banana ( index 4).
...
function runProgram(input) {
  ...
  for (let i = 0; i < tc; i++) {
    ...
    rickAndRichards(n, arr);
  }
}

function rickAndRichards(n, arr) {
  let left = 0;
  let right = n - 1;
  let rick = arr[0];
  let richa = arr[n - 1];
